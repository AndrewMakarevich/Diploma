...
class PictureCommentService {
  ...
  static async getPictureComments(
    ...
    page?: string | number,
    limit?: string | number): Promise<AxiosResponse<IGetCommentsResponseObj>> {
    const response = await $host.get<IGetCommentsResponseObj>("/api/picture-comment/get-many", { params: { pictureId, commentId, page, limit } });
